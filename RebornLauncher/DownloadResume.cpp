...
bool LauncherUpdateCoordinator::DownloadWithResume(
	...
	if (allowP2P) {
		const bool p2pDownloaded = downloader.TryP2P(strUrl, file_path, reportProgress);
		if (p2pDownloaded) {
...
bool LauncherUpdateCoordinator::EnqueueDeferredFileUpdate(
...
void LauncherUpdateCoordinator::ProcessDeferredFileUpdates() {
	DeferredFileUpdateTask task;
	{
